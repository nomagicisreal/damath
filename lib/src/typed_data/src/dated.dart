...
mixin _MixinSplayDate<K, V> on _SplayTreeMapValueList<K, V> {
  SplayTreeMap<K, List<V>> _filtered(
    ...
    for (var entry in _field.entries) {
      ...
      if (!days.bitOn(0, toDay(date))) continue;
...
class Dated<T> extends _SplayTreeMapValueList<(int, int, int), T>
  ...
  Dated.empty() : super.empty(_DateExtension.comparing);
  ...
  /// [filteredBy]
  ///
  SplayTreeMap<(int, int, int), List<T>> filteredBy(DateFlags flags) =>
      _filtered(
        flags,
        toYear: (date) => date.$1,
        toMonth: (date) => date.$2,
...
class DateTimed<T> extends _SplayTreeMapValueList<DateTime, T>
  ...
  /// [filteredByDate]
  ///
  SplayTreeMap<DateTime, List<T>> filteredByDate(DateFlags flags) => _filtered(
    flags,
