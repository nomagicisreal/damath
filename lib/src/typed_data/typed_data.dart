library damath_typed_data;

import 'dart:math' as math;
import 'dart:typed_data';

import 'package:damath/src/collection/collection.dart';
import 'package:damath/src/core/core.dart';
import 'package:vector_math/vector_math_64.dart' as v64;
import 'package:vector_math/vector_math.dart' as v32;


part 'general/constants.dart';
part 'general/dataframe.dart';
part 'general/direction.dart';
part 'general/matrix.dart';

part 'primary/double.dart';
part 'primary/integer.dart';
part 'primary/_.dart';
part 'primary/time.dart';
part 'primary/string.dart';

part 'vector/typed.dart';
part 'vector/record.dart';
part 'general/vector.dart';
part 'vector/constable.dart';