...
extension DamathIterableIterableComparable<C extends Comparable>
  ...
  ///
  bool everyElementSorted([bool increase = true]) => every(
    (sub) => sub.isSorted(DamathIteratorComparable.comparator(increase)),
  );

  void everyElementSortedThen(Listener listen, [bool increase = true]) =>
      every(
            (sub) =>
                sub.isSorted(DamathIteratorComparable.comparator(increase)),
          )
          ? listen()
